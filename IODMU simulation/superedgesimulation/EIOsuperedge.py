...
if __name__ == '__main__':
    ...
    block_matrix_seq = np.load('block_matrix_seq31.npy',allow_pickle=True)
    tempio_seq = []
    for i in range(1, len(block_matrix_seq)):
        print(i)
        tempio_value = IOMultipleLayerDMU(block_matrix_seq[0], block_matrix_seq[i], 0.000000000001)
        tempio_seq.append(tempio_value.IOMLDMU())
    #
    print(tempio_seq)

    np.save('layer31edgechange.npy',tempio_seq)
